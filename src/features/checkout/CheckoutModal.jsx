...
import { useChargePaymentMutation } from './usePaymentMutations';
...
function CheckoutModal({ show, onClose, onOrderConfirmed }) {
    ...
    const chargeMutation = useChargePaymentMutation();
    ...
    const [paymentInfo, setPaymentInfo] = useState(null); // { orderId, iframeUrl, status }
    ...
    const handleConfirm = async (e) => {
        ...
        try {
            ...
            if (paymentMethod === 'card') {
                try {
                    const payment = await chargeMutation.mutateAsync(createdOrder.orderId);
                    const iframeUrl = payment.redirectUrl
                        ? payment.redirectUrl
                        : payment.clientSecret
                            ? `https://accept.paymob.com/unifiedcheckout/?publicKey=${PAYMOB_PUBLIC_KEY}&clientSecret=${payment.clientSecret}`
                            : null;
                    ...
                    onClose();
                    setErrorMsg('Order placed, but starting payment failed. You can retry payment from "My Orders".');
                    return;
                }
            }
